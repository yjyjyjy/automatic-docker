...
launchTempateId=`aws ec2 describe-tags --filters Name=key,Values=aws:ec2launchtemplate:id Name=resource-id,Values=i-076116efb3d37ada4 | jq -r '.Tags[0].Value'`
...
echo "$(date)"' 👀 launchTempateId: '"$launchTempateId" >> /home/ubuntu/log.txt
URL=""
while true
do
  ...
  then
    ...
    cmd=$(curl -v -d '{"gradioLink":"'"$URL"'", "ip":"'"$ip"'", "ip":"'"$ip"'", "userId":"'"$userId"'", "storage":"'"$storage"'", "stackId":"'"$stackId"'", "launchTempateId":"'"$launchTempateId"'"}' -H "Content-Type: application/json" -X PUT https://a1-rouge.vercel.app/api/instance/$id)
    break
  else
    sleep 1
  fi
